...
def make_parser(template_string):
    """
    See Django's Template class in django/django/template/base.py
    for reference in constructing the test parser.
    """
    engine = Engine.get_default()
    origin = template_base.Origin('<unknown source>')
    return template_base.Parser(
        template_base.Lexer(template_string).tokenize(),
